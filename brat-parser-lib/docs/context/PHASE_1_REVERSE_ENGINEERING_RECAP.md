...
* **Le Deep Look-ahead :**
  * **Ancre A (Nouvelle Entité) :** Si le bit est 	rue, on lit l'ID (< 50) ET on valide que le short suivant correspond à une longueur de pseudo cohérente.
  * **Ancre B (Fin des Joueurs) :** Si le bit est alse, on lit le prochain State Tag (1 ou 6). Si c'est 1 (Inputs), on vérifie le bit hasInputLoop et l'ID. Si c'est 6 (Results), on vérifie que la durée du match (32 bits) est logiquement comprise entre 5 secondes et 15 minutes.
...
Tous les tests passent avec un taux de réussite de 100% sur 8/8 fichiers replay en implémentant un parseur Dry Run récursif pour l'Ancre B dans le flux, prouvant qu'il gère toutes les variantes des payloads de joueurs asymétriques/offusqués de Brawlhalla !
